<template>
  <section class="kindred-page">
    <template v-for="(item, i) in mediaItems" :key="i">
      <img
        v-if="item.type === 'image'"
        :src="item.src"
        :alt="`Kindred ${i + 1}`"
        class="kindred-img"
      />
      <video
        v-else
        :src="item.src"
        class="kindred-video"
        autoplay
        muted
        playsinline
        loop
      />
    </template>
  </section>
</template>

<script setup>
const imageModules = import.meta.glob('../assets/imgs/kindred/*.{png,jpg,jpeg,webp,gif}', { eager: true, as: 'url' })
const videoModules = import.meta.glob('../assets/imgs/kindred/*.{mp4,mov}', { eager: true, as: 'url' })

function getOrder(path) {
  const name = path.replace(/^.*[\\/]/, '').replace(/\.[^.]+$/, '')
  const num = parseInt(name, 10)
  return isNaN(num) ? 999 : num
}

const imageItems = Object.entries(imageModules).map(([path, src]) => ({
  type: 'image',
  src,
  order: getOrder(path)
}))
const videoItems = Object.entries(videoModules).map(([path, src]) => ({
  type: 'video',
  src,
  order: getOrder(path)
}))

const mediaItems = [...imageItems, ...videoItems].sort((a, b) => a.order - b.order)
</script>

<style scoped>
.kindred-page {
  width: 100%;
  min-height: 100vh;
  padding: 0;
  margin: 0;
}

.kindred-img,
.kindred-video {
  display: block;
  width: 100%;
  height: auto;
  margin: 0;
  padding: 0;
  vertical-align: top;
}
</style>
